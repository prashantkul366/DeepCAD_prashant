# %%writefile /content/DeepCAD_prashant/model/jepa_encoder.py
import torch
...
class JEPAEncoder(nn.Module):
    ...
    Returns per-token embeddings (S, N, d_model) — not mean-pooled.
    Replaces masked positions with a learnable mask_embedding before
    the transformer sees the sequence.
    """
    def __init__(self, cfg):
        ...
        self.embedding = CADEmbedding(
            cfg, cfg.max_total_len, use_group=self.use_group
        )
    ...
    def forward(self, commands, args, target_mask=None):
        ...
        target_mask: (N, S)         bool — True = position to mask (JEPA target)

        Returns: memory (S, N, d_model) — seq-first, per-token embeddings
        """
        # Convert to seq-first for transformer internals
        ...
        group_mask = (
            _get_group_mask(commands_sf, seq_dim=0)
            if self.use_group else None
        )

        # Embed all positions (including targets — we replace them next)
        src = self.embedding(commands_sf, args_sf, group_mask)  # (S, N, d_model)

        # Replace target positions with the learned mask embedding
