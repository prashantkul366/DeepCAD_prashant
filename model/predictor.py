...
class JEPAPredictor(nn.Module):
    ...
    Only target positions contribute to loss — handled outside this module.
    """
    def __init__(self, d_model, pred_dim, n_layers, n_heads, dim_feedforward, dropout=0.1):
        super().__init__()
        self.input_proj  = nn.Linear(d_model, pred_dim)
        pred_layer = TransformerEncoderLayerImproved(pred_dim, n_heads, dim_feedforward, dropout)
        self.transformer = TransformerEncoder(pred_layer, n_layers, LayerNorm(pred_dim))
...
class HierarchicalPredictor(nn.Module):
    ...
    def __init__(self, d_model, pred_dim, n_layers, n_heads, dim_feedforward, dropout=0.1):
        ...
        self.heads = nn.ModuleDict({
            level: JEPAPredictor(d_model, pred_dim, n_layers, n_heads, dim_feedforward, dropout)
